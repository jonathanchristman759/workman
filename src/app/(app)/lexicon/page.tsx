...
const CHAPTER_COUNTS: Record<string, number> = {
  ...
  'Jeremías':52,'Lamentaciones':5,'Ezequiel':48,'Daniel':12,'Oseas':14,
  'Joel':3,'Amós':9,'Abdías':1,'Jonás':4,'Miqueas':7,'Nahúm':3,
  'Habacuc':3,'Sofonías':3,'Hageo':2,'Zacarías':14,'Malaquías':4,
...
export default function LexiconPage() {
  ...
  book:        searchParams.get('book')    ?? localStorage.getItem('lexicon_book')    ?? 'John',
  chapter:     searchParams.get('chapter') ?? localStorage.getItem('lexicon_chapter') ?? '10',
  translation: language === 'ES' ? 'RVR60' : 'KJV',
})
useEffect(() => {
  localStorage.setItem('lexicon_book',    passage.book)
  localStorage.setItem('lexicon_chapter', passage.chapter)
...
setSearchResults(data)
  ...
  const maxChapter = CHAPTER_COUNTS[newBook] ?? 1
  const currentChapter = parseInt(passage.chapter)
  setPassage((p) => ({
    ...p,
    book: newBook,
    chapter: currentChapter > maxChapter ? '1' : p.chapter,
  }))
}}
              style={selectStyle}
            >
...
>
  {Array.from({ length: CHAPTER_COUNTS[passage.book] ?? 1 }, (_, i) => i + 1).map((c) => (
    <option key={c} value={c}>{language === 'ES' ? 'Cap.' : 'Ch.'} {c}</option>
  ))}
</select>

{passageData && passageData.verses.length > 0 && (
  <select
    value={passage.verse ?? ''}
    onChange={(e) => {
      const verseEl = document.getElementById(`verse-${e.target.value}`)
      verseEl?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }}
    style={{ ...selectStyle, width: 72 }}
  >
    <option value="">{language === 'ES' ? 'v.' : 'v.'}</option>
    {passageData.verses.map((v) => (
      <option key={v.verse} value={v.verse}>
        {language === 'ES' ? 'v.' : 'v.'} {v.verse}
      </option>
    ))}
  </select>
)}
            ...
            {view === 'interlinear' && searchResults.length === 0 && (
              <div>
                ...
                {loadingPassage ? (
                  ...
                  passageData.verses.map((v) => (
                    <div key={`${v.chapter}-${v.verse}`} id={`verse-${v.verse}`} style={{ marginBottom: 20 }}>
                      <p style={{
                        fontSize:      11,
                        fontWeight:    500,
